...
//     // hauteur du mur
//     map->height = (par->map->unit_v_mini / dist) * (WIDTH / 2);
...
//         wall_x = par->player->pos_y + dist * ray_y;
//     else
//         wall_x = par->player->pos_x + dist * ray_x;
//     wall_x = wall_x - floor(wall_x);
//     tex_x = (int)(wall_x * (double)map->wall_no->width);
//     if ((cos_angle) > (sin_angle) && ray_x > 0)
//         tex_x = map->wall_no->width - tex_x - 1;
//     else if (cos_angle <= sin_angle && ray_y < 0)
//         tex_x = map->wall_no->width - tex_x - 1;
//     // if ((cos_angle) > (sin_angle))
//     //     text_x = fmod(ray_y, par->map->unit_v_mini) / par->map->unit_v_mini * map->wall_no->width;
//     // else
//     //     text_x = fmod(ray_x, par->map->unit_h_mini) / par->map->unit_h_mini * map->wall_no->width;
...
//         color = *(int *)(map->wall_no->addr + text_y * map->wall_no->l_len + (int)tex_x * (map->wall_no->b_pix / 8));
...
void	draw_lines_3d(t_params *par, t_img *img, t_map *map, double start_x, int ray, int color)
{
	float	ray_x = par->player->pos_x;
	float	ray_y = par->player->pos_y;
    ...
    // pour trouver la colonne de texture
    if ((cos_angle) > (sin_angle))
		text_x = fmod(ray_y, (float)par->map->unit_v_mini) / par->map->unit_v_mini * map->wall_no->width;
    else
		text_x = fmod(ray_x, (float)par->map->unit_h_mini) / par->map->unit_h_mini * map->wall_no->width;
	// if ((cos_angle) > (sin_angle))
	// 	text_x = ((int)ray_y % par->map->unit_v_mini) / (par->map->unit_v_mini * map->wall_no->width);
    // else
	// 	text_x = ((int)ray_x % par->map->unit_h_mini) / (par->map->unit_h_mini * map->wall_no->width);
     
    dprintf(2, "cos_angle = %f, sin_angle = %f, text_x = %d, ray_x = %f, ray_y = %f\n", cos_angle, sin_angle, text_x, ray_x, ray_y);    
    incr_y = 0;
    while (start_y < end)
    {
        text_y = (int)incr_y % map->wall_no->height;
